...
set_dir
if [ -f "$DIR/gcpvariables.sh" ]; then
  source $DIR/gcpvariables.sh
fi

run_script /deploy/dev/set_environment_for_gcp.sh

[[ $* != *--skip-build* ]] && run_script build/scripts/dockerbuild.sh

[[ $* != *--skip-terraform* ]] && run_script deploy/gcp/scripts/terraformapply.sh

run_script deploy/gcp/scripts/dockerpush.sh
run_script deploy/gcp/scripts/getkubeconfig.sh
run_script deploy/gcp/scripts/deploytokubernetes.sh
